...
export const CRM_SYSTEM_API_MAPPINGS_BY_SLUG = {
	gohighlevel: [
		...
		{
			key: 'ghl.contacts.store',
			name: 'Create contact',
			crmMethod: 'POST',
			crmPath: '/contacts/',
			crmBaseUrl: GHL_BASE,
			systemMethod: 'POST',
			systemPath: '/api/crm/contacts/add',
			routeName: 'api.crm.contacts.store',
			headersNote:
				'Integrated system: Bearer + Version 2021-07-28. Application API: Bearer (API JWT). Content-Type: application/json. Requires email or phone.',
			request: {
				query: null,
				path: null,
				body: {
		...
		{
			key: 'ghl.contacts.update',
			name: 'Update contact',
			crmMethod: 'PUT',
			crmPath: '/contacts/{contactId}',
			crmBaseUrl: GHL_BASE,
			systemMethod: 'POST',
			systemPath: '/api/crm/contacts/update',
			routeName: 'api.crm.contacts.update',
			headersNote:
